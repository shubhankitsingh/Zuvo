...
const MyBookings = () => {
  ...
  const fetchMyBookings=async ()=> setBookings(dummyMyBookingsData)
  useEffect(() => {
    fetchMyBookings()}, []);
            ...
            <div key={bookings._id} className='grid grid-cols-1 md:grid-cols-4 gap-6 p-6 border border-borderColor rounded-lg mt-5 first:mt-12'>
                ...
                <div className='md:col-span-2'>
                  <div className='flex items-center gap-2'>
                     ...
                     <p className={`px-3 py-1 text-xs rounded-full ${bookings.status==='confirmed'?'bg-green-400/15 text-green-600':'bg-red-400/15 text-red-800'}`}>{bookings.status}</p>
                  </div>
                  ...
                  <div className='flex items-start gap-2 mt-3'>
                  <img src={assets.location_icon_colored} alt="calender" className='w-4 h-4 mt-1' />
                  <div >
